...
def test_update_affecting_a_row_reactivates_and_skips_the_fallback_check(monkeypatch):
    ...
    assert db.reactivate_puzzle("p1") == db.PUZZLE_REACTIVATED
    cur.execute.assert_called_once_with(queries.REACTIVATE_PUZZLE, ("p1",))
...
def test_update_affecting_no_row_and_puzzle_missing_returns_not_found(monkeypatch):
    ...
    assert db.reactivate_puzzle("nope") == db.PUZZLE_NOT_FOUND
    cur.execute.assert_any_call(queries.GET_PUZZLE_BY_ID_ANY_STATE, ("nope",))
...
def test_update_affecting_no_row_and_puzzle_exists_returns_already_active(monkeypatch):
    ...
    assert db.reactivate_puzzle("p2") == db.PUZZLE_ALREADY_ACTIVE
    cur.execute.assert_any_call(queries.GET_PUZZLE_BY_ID_ANY_STATE, ("p2",))
